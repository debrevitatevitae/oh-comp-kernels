...
def run_grid_search_cv(X, y, estimator, param_grid, cv=5, n_jobs=-3, verbose=3, error_score=np.nan):
    ...
    grid_search = GridSearchCV(
        estimator, param_grid, cv=cv, n_jobs=n_jobs, verbose=verbose, error_score=error_score)
    ...
    df_results['nan_splits'] = df_results['split0_test_score'].apply(lambda x: [
                                                                     np.isnan(x)])

    # Append to the lists in the loop
    for i in range(1, cv):
        df_results['nan_splits'] = df_results['nan_splits'] + \
            df_results[f'split{i}_test_score'].apply(lambda x: [np.isnan(x)])

    # If the majority of the test scores are NaNs, then the mean and standard deviation of the test scores are set to NaN
    df_results['mean_test_score'] = df_results.apply(lambda row: np.nan if np.sum(
        row['nan_splits']) >= cv/2 else row['mean_test_score'], axis=1)

    df_results['std_test_score'] = df_results.apply(lambda row: np.nan if np.sum(
        row['nan_splits']) >= cv/2 else row['std_test_score'], axis=1)
